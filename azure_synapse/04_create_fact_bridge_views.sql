-- 04. FACT & BRIDGE TABLES (as VIEWS)
...
SELECT * FROM OPENROWSET(BULK 'fact_sales_agg/', DATA_SOURCE = 'goldlayer', FORMAT = 'DELTA') AS rows;

CREATE OR ALTER VIEW gold.fact_sales_partitioned AS
SELECT * FROM OPENROWSET(BULK 'fact_sales_partitioned/', DATA_SOURCE = 'goldlayer', FORMAT = 'DELTA') AS rows;

CREATE OR ALTER VIEW gold.fact_order_payments_partitioned AS
SELECT * FROM OPENROWSET(BULK 'fact_order_payments_partitioned/', DATA_SOURCE = 'goldlayer', FORMAT = 'DELTA') AS rows;
